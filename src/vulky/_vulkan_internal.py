...
def syncronize_external_computation():
    if __CUDA_IS_AVAILABLE__:
        _torch.cuda.synchronize(__CUDA_DEVICE__)
...
class ResourceData:
    ...
    def copy(w_device: 'DeviceWrapper', src, src_slice, dst, dst_slice):
        # _torch.cuda.synchronize()  # just in case any pending torch write to gpu shared
...
class ResourceWrapper:
    ...
    def __init__(self,
                 ...
                 resource_slice: _typing.Dict[str, int] = None
                 ):
    ...
    def load(self, w_device, src):
        ...
        if isinstance(src, list):
            if (self.resource_data.vk_description.usage & VK_BUFFER_USAGE_INDEX_BUFFER_BIT) == VK_BUFFER_USAGE_INDEX_BUFFER_BIT:
                src = _np.array(src, dtype=_np.int32())
            else:
                src = _np.array(src, dtype=_np.float32())
        ...
        if hasattr(src, "__cuda_array_interface__"):
            return self._load_from_torch(w_device, _torch.as_tensor(src))
        if hasattr(src, "__array_interface__"):
            return self._load_from_torch(w_device, _torch.from_numpy(_np.asarray(src)))
        try:
            a = _np.asarray(src, dtype=_np.uint8, copy=False)
            # a.setflags(write=True)
            return self._load_from_torch(w_device, _torch.from_numpy(a))
        except:
            pass
        try:
            t = _torch.frombuffer(src, dtype=_torch.uint8)  # try to wrap data with a tensor
            return self._load_from_torch(w_device, t)
        except:
            pass
        try:
            t = _torch.from_dlpack(src)
            return self._load_from_torch(w_device, t)
        except:
    ...
    def _save_to_torch(self, w_device, t: _torch.Tensor):
        if self.support_direct_tensor_map:
    ...
    def save(self, w_device, dst):
        ...
        if isinstance(dst, _torch.Tensor):
            return self._save_to_torch(w_device, dst)
        if hasattr(dst, "__cuda_array_interface__"):
            t = _torch.as_tensor(dst)
            return self._save_to_torch(w_device, t)
        if hasattr(dst, "__array_interface__"):
            n = _np.array(dst, copy=False)
            t = _torch.from_numpy(n)
            return self._save_to_torch(w_device, t)
        try:
            t = _torch.frombuffer(dst, dtype=_torch.uint8)  # try to wrap data with a tensor
            return self._save_to_torch(w_device, t)
        except:
            pass
        try:
            t = _torch.from_dlpack(dst)  # try to wrap data with a tensor
            return self._save_to_torch(w_device, t)
        except:
...
class DescriptorSetWrapper:
    def __init__(self, w_device, set: int, descriptor_set_handle: int,
                 descriptor_types: _typing.Dict[int, int]):
        self.w_device = w_device
        self.set = set
        self.descriptor_set_handle = descriptor_set_handle
        self.descriptor_types = descriptor_types

    def _resolve_buffer_descriptor(self, buffer: _typing.Union[ResourceWrapper, None]):
        if buffer is None:
            # NULL DESCRIPTOR
...
class DescriptorSetCollectionWrapper:
    ...
    def __init__(self, w_device,
                 ...
                 variable_size_value: _typing.Optional[int] = None,
                 ):
        """
...
class FrameBufferWrapper:
    def __init__(self,
                 ...
                 pipeline: 'PipelineWrapper',
                 images: _typing.List[_typing.Union['ResourceWrapper', None]],
                 width: int,
                 height: int,
                 layers: int
                 ):
...
class PipelineWrapper:
    ...
    def vertex_binding(self, binding: int, stride: int, atts: _typing.Dict[int, int]):
    ...
    def add_constant_range(self, offset: int, layout: Layout):
        ...
        range_backbuffer = memoryview(_np.ndarray(layout.aligned_size, dtype=_np.uint8))
    ...
    def create_descriptor_set_collection(self,
                        set: int,
                        count: int,
                        variable_size: _typing.Optional[int] = None) -> DescriptorSetCollectionWrapper:
...
class CommandListState(_enum.Enum):
    NONE = 0
    INITIAL = 1
    RECORDING = 2
    EXECUTABLE = 3
...
class CommandBufferWrapper:
    ...
    def __init__(self, vk_cmdList, pool : 'CommandPoolWrapper'):
        ...
        self.current_pipeline : _typing.Optional['PipelineWrapper'] = None
...
class DeviceWrapper:
    ...
    def __createQueues(self):
        ...
        if __VERBOSE__: print("[INFO] Device and queues created...")
    ...
    def __createInstance(self):
        ...
        if __VERBOSE__: print(f"[INFO] Initial available layers: {self.__layers}")
        ...
        if __VERBOSE__: print("[INFO] Vulkan Instance created...")
    ...
    def __createPhysicalDevice(self, device_index: int):
        ...
        self.support_buffer_map = _torch.cuda.is_available()  # right now direct vulkan-torch interop is only valid for cuda
        ...
        if __VERBOSE__: print(f"[INFO] Supported features {dict(coop_mat=self.support_cooperative_matrices, rt=self.support_raytracing, rq=self.support_raytracing_query, atomic=self.support_atomic_float_add)}")
    ...
    def create_opengl_texture_from_vk(self, image_data):
        ...
        color = _np.random.uniform(0.0, 1.0, size=(32,32,4))
