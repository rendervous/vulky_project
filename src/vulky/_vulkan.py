...
from vulky.src.vulky._vulkancache import ffi
...
def _wrap_vkGetPhysicalDeviceCooperativeMatrixPropertiesNV(fn):
    ...
    physicalDevice
        ...
        for pindex in range(pPropertyCount[0]):
            pProperties[pindex].sType = VK_STRUCTURE_TYPE_COOPERATIVE_MATRIX_PROPERTIES_NV
        result = _callApi(fn, physicalDevice,pPropertyCount,pProperties)
        if result != VK_SUCCESS:
            raise exception_codes[result]
...
def vkMapMemory_ptr(device, memory, offset, size, flags):
    ppData = ffi.new('void**')
