...
class VulkanMemoryPage:
    def __init__(self, vk_device, capacity, memory_index: int, is_cpu: bool, is_gpu: bool):
        ...
        if is_gpu:
            ...
            if _torch.cuda.is_available():  # In a future to support other Non-nvidia general gpu implementations we need a switch here
        ...
        for t in [_torch.int32, _torch.float32, _torch.uint8, _torch.int64]:
            size = Layout.scalar_size(t)
            if is_cpu:
                self.memory_as_tensor[t] = ViewTensor.from_blob(self.memory_cpu_ptr, (capacity // size,), t,
                                                                __CPU_DEVICE__, owner=self).as_subclass(_torch.Tensor)
            else:
                if self.support_cuda:
                    self.memory_as_tensor[t] = ViewTensor.from_blob(self.memory_cuda_ptr, (capacity // size,), t,
                                                                    __CUDA_DEVICE__, owner=self).as_subclass(_torch.Tensor)
        if is_cpu or self.support_cuda:
            self.memory_as_tensor[float] = self.memory_as_tensor[_torch.float32]
            self.memory_as_tensor[int] = self.memory_as_tensor[_torch.int32]
    ...
    def load_from_cpu_ptr(self, dst_offset: int, src_ptr: int, size: int):
        assert self.is_cpu
        _ctypes.memmove(self.memory_cpu_ptr + dst_offset, src_ptr, size)
    ...
    def destroy(self):
        ...
        if self.is_gpu:
            ...
            # if _torch.cuda.is_available():  # Change in future if other general gpu memory is used
